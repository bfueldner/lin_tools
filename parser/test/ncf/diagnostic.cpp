...
TEST(test_lin_ncf_diagnostic_parser, nad_range)
{
    ...
    auto result   = phrase_parse(position, text.end(), parser::nad, x3::ascii::space, nad);
    ASSERT_TRUE(result);
...
TEST(test_lin_ncf_diagnostic_parser, diagnostic)
{
    ...
    auto position = text.begin();
    auto result =
        phrase_parse(position, text.end(), parser::diagnostic, x3::ascii::space, diagnostic);
    ASSERT_TRUE(result);
    ASSERT_EQ(position, text.end());
    ...
    EXPECT_FALSE(diagnostic.n_as_timeout);
    EXPECT_FALSE(diagnostic.n_cr_timeout);
    EXPECT_EQ(diagnostic.support_sids, (std::vector< int >{ 0xB0, 0xB2, 0xB7 }));
