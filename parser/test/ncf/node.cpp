...
TEST(test_lin_ncf_node_parser, node)
{
    ...
    auto result   = phrase_parse(position, text.end(), parser::node, x3::ascii::space, node);
    ASSERT_TRUE(result);
    ASSERT_EQ(position, text.end());

    EXPECT_STREQ(node.name.c_str(), "step_motor");
    ...
    EXPECT_STREQ(node.status_management.response_error.c_str(), "error_bit");
    EXPECT_STREQ(node.status_management.fault_state_signals.front().c_str(), "fault_state");
    EXPECT_STREQ(
