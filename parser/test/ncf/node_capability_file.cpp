...
TEST(test_lin_ncf_parser, node_capability_file)
{
    ...
    ASSERT_EQ(node_capability_file.nodes.size(), 1);
    EXPECT_STREQ(node_capability_file.nodes[0].name.c_str(), "step_motor");

    EXPECT_STREQ(
        node_capability_file.nodes[0].free_text.value.c_str(),
